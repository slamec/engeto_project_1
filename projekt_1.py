...
# creates list from the texts
    ...
    if user_pass in user_login[user_name]:
        ...
        def text_analyze(chosen_text):
            """Analyzes given text and returns statistics. Accepts one parameter"""

            # number of words
            print(f'There are {len(text[chosen_text].split())} words in the selected text.')

            # number of words starting with titlecase
            count_title = 0 
            for i in text[chosen_text].split():
                if i.istitle():
                    count_title += 1
            ...
            for d in text[chosen_text].split():
                if d.isdigit():
        ...
        if text_number <= len(text_list):
            text_analyze(text_number)
